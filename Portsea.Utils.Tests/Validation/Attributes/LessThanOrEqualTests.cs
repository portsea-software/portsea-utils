...
namespace Portsea.Utils.Tests.Validation.Attributes
{
    public class LessThanOrEqualTests
    {
        ...
        {
            // Assert
            Assert.IsFalse(this.GetAttribute(pivotValue).IsValid(value));
        }

        [Test]
        public void Check_Greater_Values_Return_False_Decimal()
        {
            // Arrange
        ...
        public void Check_Equal_Values_Return_True(double pivotValue, object value)
        {
            // Assert
            Assert.IsTrue(this.GetAttribute(pivotValue).IsValid(value));
        }

        [Test]
        public void Check_Equal_Values_Return_True_Decimal()
        {
            // Arrange
            double pivotValue = 0.00;
            decimal value = 0.00M;
        ...
        [Test]
        public void Check_Lesser_Values_Return_True_Decimal()
        {
            // Arrange
            double minimumValue = 1.00;
            decimal value = 0.00M;

            // Assert
            Assert.IsTrue(this.GetAttribute(minimumValue).IsValid(value));
        }

        private ValidationAttribute GetAttribute(double pivotValue)
        {
            return new LessThanOrEqualAttribute(pivotValue);
        }
    }
}
